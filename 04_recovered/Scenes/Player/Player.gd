...
func _ready() -> void :
				stats.connect("health_changed", self, "_on_update_healthbar")
				stats.connect("died", self, "_on_death")
				stats.connect("damage_taken", self, "_on_damage")
				stats.connect("orb_pickup", self, "_on_orb")
				stats.connect("powerup", self, "_on_powerup")
				stats.connect("vulnerable", self, "_on_vulnerable_change")
				stats.connect("shielded", self, "_on_shielded")

				
				GameState.connect("passives_changed", stats, "_on_passives_changed")
				GameState.connect("tree_changed", stats, "_on_passives_changed")
				GameState.connect("gene_loadout_changed", stats, "_on_loadout_changed")
				Genes.connect("gene_edited", stats, "_on_loadout_changed")
				...
				GameState.connect("skill_loadout_changed", self, "_on_skills_changed")

				GameState.connect("specialization_changed", self, "_on_spec_changed")
				GameState.connect("class_changed", self, "_on_class_changeds")

				stats.connect("status_effect_changed", self, "_on_stats_changed")

				Globals.connect("run_time_expired", self, "_on_time_expired")

				gear.connect("child_entered_tree", self, "_on_gear_changed")
				gear.connect("child_exiting_tree", self, "_on_gear_changed")
...
func _on_levelup() -> void :
				...
				var expl = levelup_effect.instance()
				expl.emitting = true
				add_child(expl)
				...
				for slot in equipped_skills:
								...
								if skill_name != null:
												...
												if equipped_already.has(skill_name):
																...
																var skill_inst = skill_config.skill_scene.instance()
