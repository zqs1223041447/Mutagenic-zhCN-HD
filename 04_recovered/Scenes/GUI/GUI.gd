...
func _ready() -> void :
				Globals.connect("context_changed", self, "_update_context")
				Globals.connect("context_entity_changed", self, "_update_context_entity")
				Globals.connect("show_notification", self, "_render_notification")
				Globals.connect("show_message", self, "show_message")

				GameState.connect("account_xp_changed", self, "_update_mutation_xp")
				GameState.connect("mutation_tier_increased", self, "_update_mutation_tier")
				GameState.connect("settings_changed", self, "_on_settings_changed")
				GameState.connect("stage_completion_changed", self, "_on_stage_completed")
				GameState.connect("passives_changed", self, "_update_mutation_tier")
				Genes.connect("gene_edited", self, "_update_orbs")

				$NotificationContainer / NotificationList.connect("child_exiting_tree", self, "_on_remove_child")
				$NotificationContainer / NotificationList.connect("child_entered_tree", self, "_on_add_child")
...
func bind_player():
				...
				if next_player != player:
								...
								stats.connect("health_changed", self, "_update_globe")
								stats.connect("status_effect_changed", self, "_render_buffs")
								stats.connect("damage_taken", self, "_render_damage_notification")
								stats.connect("kill_change", self, "_on_stats_changed")
								stats.connect("orb_pickup", self, "_render_pickup_notification")
								stats.connect("orb_pickup", self, "_update_orbs")
								player.connect("gear_changed", self, "_on_gear_changed")
												...
												if effect.texture == null:
																...
																if effect.stack_group:
																				if stackable_buffs.has(effect.stack_group):
																								...
																								var buff = buff_display.instance()
																								buff.effect = effect
...
func _on_gear_changed():
				...
				for item in sorted_gear:
								if item.is_queued_for_deletion():
												continue
								var display = skilldisplay.instance()
...
func _render_damage_notification(amounts, attacker_stats, was_crit):
				var content = message_scene.instance()
				...
				for i in range(len(types)):
								...
								content.add_text(str(stepify(amounts[type], 0.1)))
								content.pop()
								if i < len(amounts) - 1:
												content.add_text(" +")
...
func _update_context_entity(entity):
				...
				if entity:
								...
								entity.stats.connect("status_effect_changed", self, "update_flags")
...
func update_flags():
				...
				for flag in flags:
								if StatusEffects.should_show_flag(flag):
												var child = status_display.instance()
												child.flag = flag
