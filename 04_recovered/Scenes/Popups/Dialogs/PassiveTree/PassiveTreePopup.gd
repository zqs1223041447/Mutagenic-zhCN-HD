...
func _ready() -> void :
				...
				GameState.connect("passives_changed", self, "resync")
				GameState.connect("changed", self, "_on_settings_change")
				GameState.connect("settings_changed", self, "_on_settings_change")
				GameState.connect("tree_changed", self, "_on_tree_changed")
				rebuild()
				_on_settings_change()
				_on_tree_changed()
...
func create_nodes():
				for node in PassiveTreeData.tree_data.nodes:
								var passive_node = passive_node_scene.instance()
...
func _on_LoadButton_pressed() -> void :
				...
				popup.connect("destroy", self, "_refocus_back")
...
func _on_NewButton_pressed() -> void :
				...
				popup.connect("text_entered", self, "_create_tree")
