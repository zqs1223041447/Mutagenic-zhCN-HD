...
func _ready() -> void :
				Globals.connect("change_input", self, "_on_input_changed")
				yield(FrameTimer.idle_frame(self), "timeout")
				_zoom_at_point(max_zoom, Vector2.ZERO)
				position += get_center()
...
func _zoom_at_point(zoom_change, mouse_position):
				...
				if old_zoom != scale.x:
								var delta_x = (mouse_position.x - position.x) * (zoom_change - 1)
								var delta_y = (mouse_position.y - position.y) * (zoom_change - 1)
