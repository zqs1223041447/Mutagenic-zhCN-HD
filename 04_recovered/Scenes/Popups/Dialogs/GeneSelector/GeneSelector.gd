...
func _ready():
				GameState.connect("settings_changed", self, "render")
...
func render_available_genes(ids: Array):
				...
				for id in ids:
								var opt = gene_option.instance()
								opt.gene_id = id
								opt.connect("selected", self, "_on_select")
								opt.connect("focused", self, "_render_gene_stats")
