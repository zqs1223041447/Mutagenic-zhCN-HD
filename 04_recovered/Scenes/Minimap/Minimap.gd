...
func _ready() -> void :
				level.connect("map_done", self, "_render_map")
				MapMods.connect("mods_changed", self, "_render_mods")
				_render_mods()
...
func _render_mods():
				...
				if Globals.stage_iiq > 0.0:
								var label = mod_label.instance()
								label.text = str(stepify(Globals.stage_iiq * 100.0, 1)) + "% Increased Quantity of Items Found"
								label.align = Label.ALIGN_RIGHT
								mod_list.add_child(label)
								$PanelContainer.visible = true
				...
				if Globals.stage_iir > 0.0:
								var label = mod_label.instance()
								label.text = str(stepify(Globals.stage_iir * 100.0, 1)) + "% Increased Rarity of Items Found"
								label.align = Label.ALIGN_RIGHT
								mod_list.add_child(label)
								$PanelContainer.visible = true
