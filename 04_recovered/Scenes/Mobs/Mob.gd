...
export (bool) var disable_when_out_of_view = true
...
func _ready() -> void :
				...
				if randf() < 0.01:
								...
								var plague = plague_clouds.instance()
				...
				stats.connect("stats_changed", self, "recache_ms")
				recache_ms()
...
func _on_death():
				...
				if Levels.is_current_level_ladder():
								...
								if randf() < drop_rate or not GameState.is_tutorial_event_done("first_gene"):
												...
												var pickup = gene_pickup.instance()
								...
								if randf() < drop_rate or is_elite:
												...
												if is_elite:
																if randf() < 0.7:
																				...
																				ground_layer.add_child(pickup)
																				pickup.global_position = global_position + get_random_offset()
												var pickup = orb_pickup.instance()
												pickup.zone_level = effective_level
								...
								if is_level_boss:
												...
												var portal = portal_pickup.instance()
...
func spawn_death_animation():
				var dissolve = dissolve_sprite.instance()
				...
				if stats.status_flags.has(Constants.StatusFlags.FROZEN):
								var shatter_instance = shatter.instance()
