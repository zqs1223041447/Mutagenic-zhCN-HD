...
func _ready() -> void :
				...
				stats.connect("stats_changed", self, "recompute_chances")
				stats.connect("stats_changed", self, "reset_output_cache")
				stats.connect("stats_changed", self, "_handle_stat_change")
...
func render_supports():
				if Skills.config[self.name].playable:
								...
								return PoolStringArray(supports).join(", ")
				else:
								return ""
