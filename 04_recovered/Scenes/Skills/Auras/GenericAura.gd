...
export (Texture) var status_effect_texture = null
var unique_aura_id
var buffs_and_nerfs = {}
...
func _initialize():
				get_parent().get_parent().stats.connect("stats_changed", self, "_update_radius")
				get_parent().get_parent().stats.connect("stats_changed", self, "reapply_aura_effect")
				get_parent().get_parent().stats.connect("stats_changed", self, "recheck_enabled")
				reapply_aura_effect()
				_update_radius()
...
func get_aura_effect():
				var effect = aura_effect.instance()
...
func _physics_process(delta: float) -> void :
				$AnimatedSprite.rotation += rotation_multiplier * delta
				if not enabled:
								return
