...
func get_tier_chance(tier, max_tiers):
				var max_value = (max_tiers) * (max_tiers + 1) / 2
				return stepify(100.0 * (max_tiers - tier) / max_value, 0.1)
...
func get_tier_bounds(mod_id, tier, quality = 1.0):
				...
				var tiered_min = stepify(quality * min_value * pow(step, tier), mod_config.stepified)
				var tiered_max = stepify(quality * min_value * pow(step, tier + 1), mod_config.stepified)
...
func calculate_effective_stat(mod, quality = 1.0):
				...
				var stat = {
								...
								"amount": stepify(bounds.min + (bounds.max - bounds.min) * mod.tier_strength, mod_config.stepified)
				}
