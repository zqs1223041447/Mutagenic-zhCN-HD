...
func _ready():
				...
				if Constants.USE_STEAM:
								Steam.connect("file_read_async_complete", self, "_on_load")
								Steam.connect("file_write_async_complete", self, "_on_save")
...
func compute_stamp(stats):
				...
				to_hash.erase("stamp")
				var stamp = (str(timestamp) + "/" + JSON.print(to_hash, "", true)).sha256_text()
				return stamp
				...
				if Constants.USE_STEAM:
								...
								if f.file_exists(save_file):
												...
												if json.error == OK and typeof(json.result) == TYPE_DICTIONARY:
																var modded = false
																if not verify_stamp(json.result):
																				modded = true
																migrate(saved_stats, json.result)
																clean_saved_data()
																print("LOADED AND MERGED", JSON.print(saved_stats))
																if modded:
