...
> 只读索引：定位 `04_recovered` 里 525 个 `.gd`。给 P1-X1 / P1-X3 扫描用。  
> 不是任务入口，不是生产输入。当前主线见 `AGENT.MD`。  
> 路径与函数列表为机器提取 FACT；职责摘要是按函数名推断（INFERENCE_HIGH）。生成日期 2026-08-14。  
> 统计：Globals 185 / Scenes 326 / addons 14 = 525。
