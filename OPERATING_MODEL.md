# OPERATING_MODEL.md — 多 Agent、Task、Git 与工作区实践
...
| 角色 | 技能/身份 | 职责 |
|------|-----------|------|
| **Coordinator（主 AGENT）** | **gork** | 校准状态、生成 Task DAG、选择 Worker、分配 worktree、Review/Merge、状态与 Evidence 更新、清理、生成下一批、决定何时请 gpt 会诊 |
...
Rollback
Handoff Format
Preferred Worker（默认 AGY；可指定 opencode）
Needs Expert（可选，标记是否建议先请 gpt 会诊）
```

没有明确 Acceptance Criteria 和 Allowed/Forbidden Paths 的任务不得开始大规模修改。

gork 在分配时可根据当前 AGY 额度、任务复杂度、历史失败次数动态决定实际 Worker 与是否先请 gpt。
...
清理流程建议：发现 → 分类 → 检查引用 → 晋升 Evidence → dry-run → 删除 → 验证 → 记录。
...
当前必需：Git、Python ≥ 3.11、Godot 4.7.1 stable。

渐进采用（不要提前）：
- Godot MCP：Product 工程可启动后
- Stagehand：第一可玩 Godot 4 baseline 后
- Blender / LimboAI / 复杂 DB：真正需要时再引入
