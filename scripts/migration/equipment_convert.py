#!/usr/bin/env python3
"""P1-WAVE-H: copy+convert Equipment assets from 04_recovered into product.

Never writes 03_raw/** or 04_recovered/**. Unlike skill_convert this module
hardcodes no source directories: the caller passes the recovered-relative
roots to copy (e.g. ("Scenes/Equipment",)) because the concrete equipment
scene layout is decided by a parallel investigation task and handed over by
...
def copy_and_convert_equipment(
    recovered_root: Path,
    product_root: Path,
    roots: Iterable[str],
    ...
    roots are recovered-relative directories (e.g. ("Scenes/Equipment",));
    ...
    if after != before:
        raise RuntimeError("04_recovered was modified by equipment conversion")
