#!/usr/bin/env python3
"""P1-WAVE-J: copy+convert Interactables assets from 04_recovered into product.

Never writes 03_raw/** or 04_recovered/**. Like levels_convert this module
hardcodes no source directories: the caller passes the recovered-relative
roots to copy (e.g. ("Scenes/Interactables",)) because the concrete
interactable scene layout is decided by a parallel investigation task and
handed over by the integrator. Within every root the rules are fixed:
...
def copy_and_convert_interactables(
    recovered_root: Path,
    product_root: Path,
    roots: Iterable[str],
    ...
    roots are recovered-relative directories (e.g. ("Scenes/Interactables",));
    ...
    if after != before:
        raise RuntimeError("04_recovered was modified by interactables conversion")
