...
func on_expire():
				...
				if chains > 0:
								...
								if closest:
												...
												var effect = chain_lightning.instantiate()
												...
												var inst = chain_lightning_effect.instantiate()
												...
												if chains > 0:
																...
																for enemy in potential_targets:
																				if enemy == target:
																								continue
																				effect = chain_lightning.instantiate()
																				effect.skill_parent_weakref = skill_parent_weakref
																				effect.applier_stats_weakref = applier_stats_weakref
																				effect.damage_bundle = damage_bundle
																				effect.chains = 0
																				...
																				inst = chain_lightning_effect.instantiate()
																				inst.source_target = target_stats
