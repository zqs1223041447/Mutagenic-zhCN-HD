				...
				if applier_stats:
								...
								if applier_stats.keystones.has("TREE_MAGMATIC_BLOOD"):
												bleed_damage_per_rate = {
																"damage": {SkillTags.Tags.FIRE: time_warp_multi * 0.05 * hit_damage * ailment_effect * dot_multi / Constants.AILMENT_RATE}, 
												}
								else:
												bleed_damage_per_rate = {
																"damage": {SkillTags.Tags.PHYSICAL: time_warp_multi * 0.05 * hit_damage * ailment_effect * dot_multi / Constants.AILMENT_RATE}, 
												}
								var amount = 0
...
func get_status_flags():
				return [Constants.StatusFlags.BLEEDING]

func get_remaining_bleed_damage():
				var effective_remaining = bleed_damage_per_rate.duplicate()
				for k in bleed_damage_per_rate.damage:
								effective_remaining.damage[k] *= (lifetime - lifetime_expired) * Constants.AILMENT_RATE
