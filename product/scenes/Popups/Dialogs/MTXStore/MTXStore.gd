...
func _ready() -> void :
				...
				MtxManager.connect("inventory_changed", Callable(self, "_render_shop"))
				MtxManager.connect("shop_changed", Callable(self, "_render_shop"))
				_render_shop()
...
func _render_shop():
				...
				for item in MtxManager.get_all_mtx():
								var shop_item = mtx_item.instantiate()
