...
@onready var gene_list = $GeneInventory/CenterContainer/PanelContainer/VBoxContainer/ScrollContainer2/Items
...
func _ready():
				...
				render_genes()
				Genes.connect("genes_changed", Callable(self, "render_genes"))
				GameState.connect("genes_changed", Callable(self, "render_genes"))
...
func render_genes():
				...
				for slot in Genes.GeneSlot.values():
								var itemlist = item_list.instantiate()
								itemlist.slot = slot
