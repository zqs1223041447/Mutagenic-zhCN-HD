...
func _ready() -> void :
				_update_name()
				GameState.connect("settings_changed", Callable(self, "_check_visibility"))
				GameState.connect("seen_items_changed", Callable(self, "update_seen_label"))
				Globals.connect("search_changed", Callable(self, "_check_search"))
...
func _on_GeneButton_pressed() -> void :
				if action_transfer:
								...
								var popup = editor.instantiate()
								popup.gene_id = gene_id
								popup.connect("destroy", Callable(self, "_select"))
								if popup_parent:
...
func _on_GeneButton_focus_entered() -> void :
				...
				tooltip.render(gene_id, global_position, size, is_in_shared)
...
func quick_delete():
				...
				if gene:
								...
								popup.connect("confirmed", Callable(self, "_on_delete_gene"))
								add_child(popup)
