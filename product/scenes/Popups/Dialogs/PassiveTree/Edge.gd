...
func _ready() -> void :
				...
				if specialization_tree:
								SpecializationTreeUtils.connect("allocation_path_changed", Callable(self, "_update_edge"))
				else:
								PassiveTreeUtils.connect("allocation_path_changed", Callable(self, "_update_edge"))
