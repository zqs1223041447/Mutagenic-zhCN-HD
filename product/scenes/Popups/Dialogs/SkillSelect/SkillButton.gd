...
func _ready() -> void :
				GameState.connect("skills_changed", Callable(self, "update_skill"))
				GameState.connect("skill_loadout_changed", Callable(self, "update_skill"))
				update_skill()
...
func render_skill(skill):
				if SlotRequirements.get_required_level_for_skill(slot) > GameState.get_account_level():
								...
								$SkillButton/VBoxContainer/NameLabel.set("theme_override_colors/font_color", Colors.nerfed)
				else:
								$SkillButton/SkillButton.disabled = false
								$SkillButton/VBoxContainer/NameLabel.set("theme_override_colors/font_color", Color.WHITE)
								if skill == null:
												$SkillButton/SkillButton/MarginContainer/TextureRect.texture = null
...
func _on_SkillButton_pressed() -> void :
				var popup = skill_list.instantiate()
				popup.connect("equip_skill", Callable(self, "_equip_skill"))
