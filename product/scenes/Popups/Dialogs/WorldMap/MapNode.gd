				...
				if node_id == "root":
								...
								if Levels.config[map_name].has("leaderboard"):
												...
												if iiq > 0:
																...
																$MapButton/StatInfoContainer/VBoxContainer/ItemQuanityLabel.text = str(snapped(iiq * 100.0, 1)) + "% Increased Quantity of Items Found"
												if iir > 0:
																$MapButton/StatInfoContainer/VBoxContainer/ItemRarityLabel.visible = true
																$MapButton/StatInfoContainer/VBoxContainer/ItemRarityLabel.text = str(snapped(iir * 100.0, 1)) + "% Increased Rarity of Items Found"
...
func render_leaderboard(leaderboard):
				...
				for entry in entries:
								...
								spec_label.horizontal_alignment = HORIZONTAL_ALIGNMENT_RIGHT
...
func set_zoom(zoom):
				$MapButton/StatInfoContainer.scale = Vector2(1.0 / zoom, 1.0 / zoom)
