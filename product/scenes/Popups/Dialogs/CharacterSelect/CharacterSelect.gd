...
func _ready() -> void :
				GameState.connect("characters_changed", Callable(self, "render"))
				render()
...
func _on_CharacterCreateButton_pressed() -> void :
				var popup = class_select.instantiate()
				popup.connect("character_created", Callable(self, "_create_character"))
