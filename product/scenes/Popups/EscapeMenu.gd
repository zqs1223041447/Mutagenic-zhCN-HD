...
func _ready() -> void :
				...
				for item in items:
								var tab_content = tab_pane.instantiate()
				...
				tab_icon_container.connect("focus_entered", Callable(self, "_focus_tab_zero"))
				_focus_tab_zero()
...
func _on_AbandonButton_pressed() -> void :
				...
				confirm_dialog.connect("confirmed", Callable(self, "_on_confirm_abandon"))
...
func update_stats():
				...
				if not showing_breakdown:
								...
								for stat in StatsInfo.character_sheet_list:
												var label = stat_scene.instantiate()
												...
												if stat in StatsInfo.damage_list:
																...
																if effective_multiplier != 1.0:
																				...
																				if stats.ga(stat) != 0.0:
																								...
																								label.stat_value = str(snapped(stats.ga(stat), 0.1))
																								stat_container.add_child(label)
