...
func _process(delta: float) -> void :
				...
				if len(queued_popups) > 0:
								...
								if is_instance_valid(popup) and is_instance_valid(node):
												popup.connect("destroy", Callable(Callable(self, "_on_destroy")).bind(popup))
...
func push_popup(popup):
				if len(open_popups) > 0:
								var next = open_popups.back()
								next[0].process_mode = PROCESS_MODE_PAUSABLE
				open_popups.append([popup, popup.process_mode])
